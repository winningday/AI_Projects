...
final class OpenAICleanupClient {
    ...
    private func buildSystemPrompt(
        ...
        if let hint = inputContextHint { prompt += "\n\nINPUT CONTEXT: \(hint)" }
        if let context = contextText, !context.isEmpty { prompt += "\n\nCONTEXT: \"\(context)\"" }
...
enum OpenAICleanupError: LocalizedError {
    case missingAPIKey, invalidResponse, noTextInResponse
