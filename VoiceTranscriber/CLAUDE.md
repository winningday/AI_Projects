...
|---------|-----------|--------------|
| `dictionaryWords` | WhisperClient prompt, DeepgramClient keywords, ClaudeClient prompt | Custom words/names for spelling accuracy |
| `styleTone` (per context) | ClaudeClient, ProgrammaticCleaner | Formal/Casual/VeryCasual/Excited formatting |
| `contextAwareness` | TextInjector reads active field, passed to Claude | Helps spell names, understand topic |
| `smartFormatting` | ClaudeClient | Preserves code formatting (camelCase, etc.) |
| `recentCorrections` | ClaudeClient | Self-learning from user edits |
| `translationEnabled` + `targetLanguage` | ClaudeClient | **Forces AI cleanup** — programmatic can't translate |
...
- **Claude does NOT support audio input via API** — do not attempt to send audio to Claude directly
...
5. **Claude does NOT support audio input** — the API only accepts text/image, never send audio to Claude.
